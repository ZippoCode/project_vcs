...
def match_keypoint(painting):
    img2 = cv2.imread('../data/paintings_db046.png', 0)
    # Initialize ORB detector
    orb = cv2.ORB_create()

    painting_gray = cv2.cvtColor(painting, cv2.COLOR_BGR2GRAY)
    # Extract keypoints and descriptors
    keypoints1, descriptors1 = orb.detectAndCompute(painting_gray, None)
    ...
    img3 = draw_matches(painting, keypoints1, img2,
                        keypoints2, matches[:40])
