...
def eight_directional_sobel_filter(image, stride=1):
    ...
    for col in range(oH):
        for row in range(oW):
            Gx = np.sum(image[col * stride: col * stride + kH, row * stride: row * stride + kW] * S_h)
            Gy = np.sum(image[col * stride: col * stride + kH, row * stride: row * stride + kW] * S_v)
            G_dl = np.sum(image[col * stride: col * stride + kH, row * stride: row * stride + kW] * S_dl)
            G_dr = np.sum(image[col * stride: col * stride + kH, row * stride: row * stride + kW] * S_dr)
            M = np.sqrt(Gx ** 2 + Gy ** 2)
...
def edge_detection(im):
    ...
    delta = 0
    ddepth = cv2.CV_16S
    grad_x = cv2.Sobel(hsv[:, :, V], ddepth, 1, 0, ksize=3, scale=scale, delta=delta, borderType=cv2.BORDER_DEFAULT)
    grad_y = cv2.Sobel(hsv[:, :, V], ddepth, 0, 1, ksize=3, scale=scale, delta=delta, borderType=cv2.BORDER_DEFAULT)
